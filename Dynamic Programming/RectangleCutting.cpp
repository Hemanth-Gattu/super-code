...
int main()
{
    ...
    // dp[i][j] stores tht minimum moves required for rectangle of size i * j
    ll dp[a+1][b+1];

    // base case: if a square no moves are required
    ...
    fll(i,1,a+1){
       ...
       fll(j,1,b+1){
           ...
           fll(k,1,i/2 +1){
               dp[i][j]=min(dp[i][j] , dp[k][j] +dp[i-k][j] + 1);  // rectangle divided into k * j and i-k * j 
           }

           fll(k,1,j/2+1){
               dp[i][j]=min(dp[i][j] , dp[i][k] + dp[i][j-k] +1);   // rectangle divided into i * k and i * j-k 
           }
       }
    }
