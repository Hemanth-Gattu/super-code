...
int main()
{
    ...
    // dp[i][j] stores whether it is possible to get sum j using the first i elements
    // dp transition : dp[i][j]=max(dp[i-1][j] , dp[i-1][j-c[i]] )
    
    
    mem0(dp);
    
    // base case: sum==0 can be achieve by any number of elements
    fll(i,0,n+1){
        dp[i][0]=1;
    }
