...
struct ST {
  ...
  int query(int v, int tl, int tr, int l, int r) {
    ...
    int mid = (tl + tr)/2; 
    int L = query(2*v, tl, mid, l, r);
    int R = query(2*v+1, mid + 1, tr, l, r);
    return max(L, R);
  }
  
  int getindex(int v , int tl , int tr, int x){
      if (t[v]<x){
          return -1;
      }
      if (tl==tr){
      ...
      int mid = (tl+tr)/2;
      if (t[2*v]>=x){
          return getindex(2*v,tl,mid,x);
      }
      else{
           return getindex(2*v+1,mid+1,tr,x);
      }
  }
};
...
int main()
{
    ios_base::sync_with_stdio(false),cin.tie(0),cout.tie(0);
     ...
     while(m--){
         cin>>r;
         int index=st.getindex(1,0,n-1,r);
         if (index==-1){
             cout<<0<<" ";
         }
         else{
             cout<<index+1<<" ";
              st.upd(1,0,n-1,index,a[index] - r);
              a[index]=a[index]-r;
         }
